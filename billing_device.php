...
session_start();
    ...
    $devices = $con->execute_query("SELECT * from enrolled_devices e JOIN service_location s ON e.location_id = s.location_id JOIN user_data u on s.user_id = u.user_id WHERE u.user_id = $user_id");
    $categories = $con->execute_query("Select * from service_location WHERE user_id = $user_id");
...
<li><a href="billing.php">Go back</a></li>
<form method="GET">
    <div>
    <label>Select time period</label>
        <!-- <select type="text" name="enrolled_device">
        <option value disabled selected>Select Enrolled Device</option>
            <?php
                    include("connection.php");
                    $user_id= $_SESSION['user_id'];
                    while($c=mysqli_fetch_array($devices)){
            ?>
            <option value="<?= isset($_GET['enrolled_device_id'])==true ? $_GET['enrolled_device_id'] :'' ?>"><?php echo $c['device_type'] , $c['model_id'] ?></option>
            <?php } ?>
        </select> -->
...
<?php  
    if(isset($_GET['start_date']) && isset($_GET['end_date']) && $_GET['start_date'] !='' && $_GET['end_date'] !=''){
        ...
        SUM(eu.kwh_used * akc.price) AS amount
    FROM
        user_data u
    JOIN
    ...
    GROUP BY
        u.user_id ,ed.enrolled_device_id;
